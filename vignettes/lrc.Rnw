...
To train an elastic net model, the user needs to supply a handful of arguments to the {\tt LRCglmnet} function. The mandatory arguments are the true class labels, {\tt truthLabels} (which, in this case, is, is the {\tt response} object we created above), the matrix of predictor variables, {\tt predictors}, 
