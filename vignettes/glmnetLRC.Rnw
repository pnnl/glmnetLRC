...
\begin{figure}[H]
\begin{center}
<<plot, fig = TRUE, width = 6, height = 6>>=
plot(lrc_fit)
@
...
specified the documentation of the {\tt glmnet} package \cite{glmnet}.
...
\begin{align}
\label{eq:risk}
R(\alpha,\lambda,\tau) = \frac{\sum_{i=1}^N w_i L \Bigl(y_i, f\bigl(\mathbf{x}_i,
~\hat{\boldsymbol{\beta}}^{-\delta(i)}(\alpha,\lambda), ~\tau \bigr) \Bigr)}{\sum_{i=1}^N w_i}
\end{align}
...
\hat\lambda_j, \hat\tau_j)$ as defined by \eqref{eq:tuning_estimates}.  Using those tuning parameter estimates, a
...
~\hat{\boldsymbol{\beta}}^{-\delta_j(i)}(\hat\alpha_j,\hat\lambda_j),  ~\hat\tau_j \bigr) \Bigr)
\end{align}
