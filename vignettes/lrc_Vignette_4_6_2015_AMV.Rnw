...
\noindent {\tt LRCglmnet} requires a binary response variable, coded
...
\caption{{\sc lrc} searches the parameter space of $(\alpha, \lambda, \tau)$ for the 
